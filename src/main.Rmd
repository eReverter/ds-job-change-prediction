...
```{r}
cat = FactoMineR::catdes(df[,-c(13:14)], 12)
cat$test.chi2
cat$quanti.var
cat$category
cat$quanti
...
```{r}
m6 = step(glm(target ~ I(city_development_index^-0.5) + ., data=df, family=binomial), k = log(nrow(df)))
```
...
Try size*(educ + city)
```{r}
m7 = glm(target ~ company_size*(education_level+I(city_development_index^-0.5)), data=df, family=binomial)
...
```{r}
m1 = step(glm(target ~ ., data=df, family=binomial), k = log(nrow(df)))
avPlots(m1)
par(mfrow=c(2,2))
plot(m1) 
par(mfrow=c(1,1))

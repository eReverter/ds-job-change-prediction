...
```{r}
maux = glm(target ~ I(city_development_index^-0.5) + . - city_development_index, data=df, family=binomial)
m6 = step(maux)
vif(m6)
```
...
```{r}
library(pROC)
prob=predict(m8, type=c("response"))
df$prob=prob
...
Model diagnostics:
```{r}
residuals_p = residuals(m8, type="pearson")
residuals_d = residuals(m8, type="deviance")
residuals_s = rstudent(m8)
hat = hatvalues(m8)
cook = cooks.distance(m8)
beta = dfbetas(m8)
outlierTest(m8)
residualPlots(m8)
...
```{r}
residualPlots(m8, layout=c(1, 3))
influenceIndexPlot(m8, id.n=10)
matplot(dfbetas(m8),type='l')
abline(h=sqrt(2/(dim(df)[1])),lty=3,col=6)
abline(h=-sqrt(2/(dim(df)[1])),lty=3,col=6)
lines(sqrt(cook),lwd=3,col=1)
# legend(locator(n=1),legend=c(names(as.data.frame(dfbetas(m8))),"Cook D"), col=c(1:3,1), lty=c(3,3,3,1) )
```

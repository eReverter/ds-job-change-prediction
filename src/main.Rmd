...
To wrap up, the model has been used to predict the most representative individual (i.e experience on the mean, Graduate, having reported the company size and from a very developed city). It can be seen that the model predicts that he/she is very unlikely to change jobs. Alternatively, it can be seen that by maintaining the same parameters for the individual and changing its city to a not very developed one, the probability to change increases a lot.
```{r}
...
Overall, the project has proved the importance of the initial data treatment and how the decisions made by the data scientist that face the problems affect the final outcome of the model. It helped to consolidate the general workflow of creating a model from scratch and the assessements that have to be done in each of the steps.

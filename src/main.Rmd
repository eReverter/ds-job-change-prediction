...
AIC(m0, m1, m2, m3, m4) # m2 per anova, cert
```
...
```{r}
hat = hatvalues(mb)
hat_cut = 3*p/n #if we assume the dataset is small, then 2p/n - Utilitzem qt()?
...
mre = glm(target ~ poly(n.experience,2) + ., data=df, family=binomial)
mbest = step(mre)
```
...
Confusion matrix on the test set. First we need to apply the same transformations to the test set (we could do this beforehand):
```{r}
...
table(df$f.city_development_index)
levels(test$f.city_development_index)[test$f.city_development_index=="[0.479,0.691)"] = "[0.448,0.691)"
...
test$prob = prob
g = roc(target ~ prob, data = test)
plot(g)
auc(test$target, prob)

confusionMatrix(data = as.numeric(predicted>0.5), reference = test$target)
```

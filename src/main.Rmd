...
mnexppoly = glm(target ~ poly(n.experience,3), data=df, family=binomial)
summary(mnexppoly)
mnexppoly = glm(target ~ poly(n.experience,2), data=df, family=binomial)
...
## ----------------need to collapse, an error arises for me:(--------------------------##

mcexpcol = glm(target ~ colapsed_exp, data=df, family=binomial)


anova(mcexpcol,mcexp,test='Chisq')

AIC(mcexp,mcexpcol,mnexppoly,mnexp)
...
Model diagnostics:
```{r}
residuals_p = residuals(m11, type="pearson")
residuals_d = residuals(m11, type="deviance")
residuals_s = rstudent(m11)
hat = hatvalues(m11)
cook = cooks.distance(m11)
beta = dfbetas(m11)
outlierTest(m11)
residualPlots(m11)

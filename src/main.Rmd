...
Analyze collapsed categories looking at the pvalues of the AIC step:
```{r}
aux = df %>% 
  mutate(across(where(is.factor), ~ as.character(.))) %>%
...
Studentized Residuals: Some outliers have been found in them, but they do not have,in any case, a value greater than 3. It can be seen that they are all people who want to change jobs, and contrary to the whole dataset, the vast majority of them are only high school graduates (hence no major) and all of them had specified the company size. Moreover, performing an outlierTest only one observation is detected, but it will also be detected and removed when assessing the cook's distance.
```{r}
n = dim(df)[1]
p = mb$rank
res_mb = rstudent(mb)
...
summary(aux)
```
The influential data can be clearly seen with the help of the influence plot:
```{r}
influencePlot(mb)
```
...
The outliers detected with the Cook's distance method have been removed from the dataset, and the model has been reevaluated without those observations. The two models have been evaluated firstly with AIC model always knowing that it is not a strictly accurate comparisson as the number of observations differs from one model to the other, but as it is only in 7 observations some a general intuition of the behaviour can be obtained. It can be seen that the new reevaluated model seems to be better, and the influencePlot results are more accurate. 
```{r}
...
# Option 1 to evaluate
AIC(mb, mbest)

# Option 2 to evaluate
mb$deviance / (3701 - mb$rank)
mbest$deviance / (nrow(df) - mbest$rank)
...
The Area Under the Curve has been assessed for the test set, and a 0.5% improvement can be seen from the reevaluated model, which supports the previous conclusion that the removing the influential observations was beneficial. Regarding the value, 81.3% can be considered a very good model. 
```{r}
# mbest
...
EXTRA - Option 3 to evaluate which model is better


MBEST (MANUAL COOKS) #Aixo ara és sencer vs treient cooks no?
```{r}
prob = predict(mb, newdata = daux, type = "response")
...
MBAUX (1% COOKS)
```{r}
prob = predict(mbest, newdata = df, type = "response")
prob = ifelse(prob<0.5,0,1)
confusionMatrix(data = as.factor(prob), reference = df$target)
```


Lastly, the confusion matrix was assessed on the test set. First of all, the same transformations that have been done during the modelling steps have to be applied to the test set.
```{r}
# aux = test

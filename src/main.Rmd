...
aux3 = aux2 %>% 
  mutate(across(where(is.factor), ~ as.character(.))) %>%
  mutate(enrolled_university = case_when(enrolled_university != "no_enrollment" ~ "Enrolled",
                                  TRUE ~ enrolled_university)) %>%
  mutate(across(where(is.character), ~ as.factor(.)))

maux3 = glm(formula = target ~ poly(n.experience, 2) + relevent_experience + 
    enrolled_university + education_level + company_size + last_new_job + 
    training_hours + f.city_development_index, family = binomial, 
    data = aux3)
...
aux4 = aux3 %>% 
  mutate(across(where(is.factor), ~ as.character(.))) %>%
  mutate(last_new_job = case_when(last_new_job == "1" ~ "1",
                                  last_new_job == "never" ~ "never",
                                  TRUE ~ "[2-4]")) %>%
  mutate(across(where(is.character), ~ as.factor(.)))

maux4 = glm(formula = target ~ poly(n.experience, 2) + relevent_experience + 
    enrolled_university + education_level + company_size + last_new_job + 
    training_hours + f.city_development_index, family = binomial, 
    data = aux4)
```
...
mbic3 = step(maux3, scope = . ~ .^2, k = log(nrow(df)))
mbic4 = step(maux4, scope = . ~ .^2, k = log(nrow(df)))

AIC(maic, maux, maux2, maux3, maux4, mbic, mbic2, mbic3, mbic4) # Go with mbic2
summary(mbic2)
...
```{r}
avPlots(mbic2)
marginalModelPlots(mbic2)
residualPlots(mbic2)
allEffects(mbic2)
plot(allEffects(mbic2))
```
...
cook_cut = 4/(n-p) # Option 1
cook_cut = 0.003 # Option 2
cook_cut2 = quantile(cook, 0.99)[[1]] # Option 3

lc = Boxplot(cook, id=list(n=27))
abline(h=cook_cut)

nrow(df[which(cook>cook_cut),])
nrow(df[which(cook>cook_cut2),])
...
# With percentile cooks 
mbaux = glm(formula = target ~ poly(n.experience, 2) + education_level + 
    company_size + f.city_development_index + company_size:f.city_development_index, 
    family = binomial, data = daux)
...
mbaux$deviance / (nrow(daux) - mbaux$rank)
```
...
#mbaux
prob = predict(mbaux, type=c("response"))
...
MBAUX (1% COOKS)
```{r}
prob = predict(mbaux, newdata = daux, type = "response")
prob = ifelse(prob<0.5,0,1)
confusionMatrix(data = as.factor(prob), reference = daux$target)
```

Test data (Aixo no esta ben fet pero bueno era per curiositat)
...
MBAUX(1% COOKS)
```{r}
prob = predict(mbaux, newdata = test, type = "response")
test$prob = ifelse(prob<0.5,0,1)
confusionMatrix(data = as.factor(test$prob), reference = test$target)
```
